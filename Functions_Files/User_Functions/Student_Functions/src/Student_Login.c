...
#define CSV_Student "./DataBases/Student_DataBase.csv"

// --- Helper Function: Add Node to List ---
void add_student_node(User **head, char *ID, char *Password , char *Name) {
...
User* load_students_from_csv() {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (ID != NULL && Password != NULL && Name != NULL) {
            ...
            add_student_node(&head, ID, Password,Name);
        }
    }
...
int perform_student_login(char *ID, char *Password, char *Name) {
    ...
    User *head = load_students_from_csv();
    User *current = head;
    int success = 0;
